...
public:	
	...
	void MakeRates(EventHistory<T_time, T_amt, T_rate, T_ii>& events, int nCmt) {	
	  ...
	  vector<T_time> EventTimes(nEvent, 0);
	  vector<T_rate> rate_init(nCmt, 0);
	  Rate<T_time, T_rate> newRate(time_init,rate_init);
	
	  if(!events.Check()) events.Sort(); //events need to be sorted, else the function 
	                                     // will not compute one rate per time, but most 
	                                     // likely one rate per event. 
