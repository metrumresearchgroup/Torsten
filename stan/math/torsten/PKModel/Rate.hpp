...
public:
  ...
  Rate(T_time p_time, vector<T_rate> p_rate) {
    time = p_time;
    rate = p_rate;
  }
  ...
  friend
  Matrix<typename boost::math::tools::promote_args<T_0, T_1, T_2, T_3,
    typename boost::math::tools::promote_args<T_4, T_5>::type >::type, Dynamic, Dynamic>
...
public:	
  ...
  RateHistory(vector<T0> p_time, vector<vector<T1> > p_rate) {
    ...
    Rates.resize(nRate);
    for(int i = 0; i < nRate; i++) Rates[i] = Rate<T_time, T_rate>(p_time[i],p_rate[i]);
  }
  ...
  void Print(int j) {
    ...
    for(int i = 0; i < Rates[j].rate.size(); i++) std::cout << Rates[j].rate[i] << " ";
    std::cout << std::endl;
  }
