...
void operator()(const generalOdeModel_control_ss& fx) const {
  o_ << fx.integration_function_name_
     << '('
     << fx.system_function_name_
     << "_functor__(), ";
  ...
  generate_expression(fx.ss_rel_tol_, NOT_USER_FACING, o_);
  o_ << ", ";

  generate_expression(fx.ss_abs_tol_, NOT_USER_FACING, o_);
  o_ << ", ";
