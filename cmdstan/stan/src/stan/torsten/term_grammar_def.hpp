...
generalOdeModel_control_ss_r.name("expression");
generalOdeModel_control_ss_r
%= ( (string("generalOdeModel_bdf") >> no_skip[!char_("a-zA-Z0-9_")])
  ...
  >> lit(',')
  >> expression_g(_r1)   // 17) relative tolerance for algebra solver(data only)
  >> lit(',')
  >> expression_g(_r1)   // 18) absolute tolerance for algebra solver(data only)
  >> lit(',')
  >> expression_g(_r1)   // 19) maximum number of steps for algebra solver
  > lit(')')
  [validate_generalOdeModel_control_ss_f(_val,
