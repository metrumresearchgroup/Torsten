#ifndef STAN_MATH_TORSTEN_ONECPT_EFFCPT_HPP
#define STAN_MATH_TORSTEN_ONECPT_EFFCPT_HPP
 ...
 */
  ...
  auto pmx_solve_onecpt_effcpt(Ts... args) {
    return PMXSolveCPT<PMXOneCptEffCptModel>::solve(args...);
  }
 ...
 */
  ...
  pmx_solve_onecpt_effcpt(const std::vector<T0>& time,
    ...
    return pmx_solve_onecpt_effcpt(time, amt, rate, ii, evid, cmt, addl, ss,
...
pmx_solve_group_onecpt_effcpt(const std::vector<int>& len,
  ...
  int nCmt = torsten::PMXOneCptEffCptModel<double>::Ncmt;
  ER events_rec(nCmt, len, time, amt, rate, ii, evid, cmt, addl, ss);

  static const char* caller("pmx_solve_group_onecpt_effcpt");
  torsten::pmx_population_check(len, time, amt, rate, ii, evid, cmt, addl, ss,
                                pMatrix, biovar, tlag, caller);

  using model_type = torsten::PMXOneCptEffCptModel<typename EM::T_par>;
