...
namespace torsten {
  ...
  struct NonEventParameters<T0, T4, theta_container,
    ...
    /**
     * Get bioavailability given event id & compartment id
     *
     * @param iEvent event id
     * @param iParameter compartment id
     * @param event_ctrl event control parameter array
     *
     * @return F[i,j] (when F is a parameter), otherwise the default F (1.0).
     */
    inline const T5 bioavailability(int iEvent, int iParameter) const {
      int i = npar > 1 ? get_par_array(iEvent)[1] : -1; // use dummy index if F is not parm
      return NonEventParameters_Impl<tuple_pars_t...>::bioavailability(i, iParameter,
                                                                       event_ctrl);
    }

    /**
     * Get lag time given event id & compartment id
     *
     * @param iEvent event id
     * @param iParameter compartment id
     * @param event_ctrl event control parameter array
     *
     * @return lag[i,j] (when lag time is a parameter), otherwise the default lag time (0.0).
     */
    inline const T6 lag_time(int iEvent, int iParameter) const {
      int i = npar > 2 ? get_par_array(iEvent)[2] : -1; // use dummy index if lag is not parm
      return NonEventParameters_Impl<tuple_pars_t...>::lag_time(i, iParameter,
                                                                event_ctrl);
    }
