...
TEST_F(TorstenLinODEOneCptTest, steady_state_overloads) {
  ...
  for(int i = 0; i < amounts.rows(); i++) {
    EXPECT_NEAR(xt(0, i), x_122(0, i),
                std::max(xt(0, i), x_122(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_122(1, i),
                std::max(xt(1, i), x_122(1, i)) * 1e-6);
		
    EXPECT_NEAR(xt(0, i), x_112(0, i),
                std::max(xt(0, i), x_112(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_112(1, i),
                std::max(xt(1, i), x_112(1, i)) * 1e-6);
		
    EXPECT_NEAR(xt(0, i), x_111(0, i),
                std::max(xt(0, i), x_111(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_111(1, i),
                std::max(xt(1, i), x_111(1, i)) * 1e-6);
		
    EXPECT_NEAR(xt(0, i), x_121(0, i),
                std::max(xt(0, i), x_121(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_121(1, i),
                std::max(xt(1, i), x_121(1, i)) * 1e-6);
		
    EXPECT_NEAR(xt(0, i), x_212(0, i),
                std::max(xt(0, i), x_212(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_212(1, i),
                std::max(xt(1, i), x_212(1, i)) * 1e-6);
		
    EXPECT_NEAR(xt(0, i), x_211(0, i),
                std::max(xt(0, i), x_211(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_211(1, i),
                std::max(xt(1, i), x_211(1, i)) * 1e-6);
		
    EXPECT_NEAR(xt(0, i), x_221(0, i),
                std::max(xt(0, i), x_221(0, i)) * 1e-6);
    EXPECT_NEAR(xt(1, i), x_221(1, i),
                std::max(xt(1, i), x_221(1, i)) * 1e-6);
  }
}
...
TEST(Torsten, linOne_MultipleDoses_timePara) {
  ...
  MatrixXd system_1(2, 2), system_2(2, 2);
  system_1 << -ka, 0, ka, -k10_1;
  system_2 << -ka, 0, ka, -k10_2;
	
  vector<MatrixXd > system_array(nEvent, system_1);
...
TEST_F(TorstenLinODEOneCptTest, amt_var) {
  ...
  auto test_it = [&](Matrix<var, Dynamic, Dynamic>& res) {
    for (int i = 0; i < res.rows(); ++i) {
      res(1, i).grad(amt_v, g);
      EXPECT_NEAR(g[0], (x1(1, i) - x2(1, i))/(2 * h), 1e-6);
      stan::math::set_zero_all_adjoints();

      // for gut there is a non-smooth point, skip it
      res(0, i).grad(amt_v, g);
      EXPECT_NEAR(g[0], (x1(0, i) - x2(0, i))/(2 * h), 1e-6);
      stan::math::set_zero_all_adjoints();
    }
  };
...
TEST_F(TorstenLinODEOneCptTest, rate_var) {
  ...
  auto test_it = [&](Matrix<var, Dynamic, Dynamic>& res) {
    // test val
    torsten::test::test_val(xt, stan::math::value_of(res));

    // test adj
    for (int i = 0; i < res.rows(); ++i) {
      res(1, i).grad(rate_v, g);
      stan::math::set_zero_all_adjoints();
      EXPECT_NEAR(g[0], (x1(1, i) - x2(1, i))/(2 * h), tol);

      // skip the non-smooth location
      res(0, i).grad(rate_v, g);
      stan::math::set_zero_all_adjoints();
      if (time[i] != t_cutoff) {
        EXPECT_NEAR(g[0], (x1(0, i) - x2(0, i))/(2 * h), tol);
      }
    }
  };
