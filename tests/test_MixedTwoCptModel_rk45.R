...
run_test_MixedTwoCptModel_rk45 <- function(event, steptime) {
modelName <- "test_MixedTwoCptModel_rk45"
par <- default_stan_parameters()
return (simulation_test (event, steptime, modelName, mrgcode, par))
}

testmodel = "MixedTwoCptModel_rk45"
...
test_that(dose, {
    ...
    res <- run_test_MixedTwoCptModel_rk45(e1, time)
    fit <- res$fit
    data <- res$data
    do_tests(fit, data)
})
...
context(paste(testmodel, "with", dose))
test_that(dose, {
    e1 <- ev(amt = 80 * 1000, rate = 0, ss=1, ii=12, addl=10)
...
context(paste(testmodel, "with", dose))
test_that(dose, {
    e1 <- ev(amt = 10, rate=5, ss=1, ii=12, addl=10)
    time <- c(0, 12, 100) ## Observation times
    res <- run_test_MixedTwoCptModel_rk45(e1, time)
    fit <- res$fit
    data <- res$data
    do_tests(fit, data)
