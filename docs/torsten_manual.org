...
Torsten's functions are prefixed with =pmx_=. Many Torsten functions
have multiple signatures and support optional arguments. 
We indicate arguments that are optional by putting them in square bracket =[]=.
Torsten also adopts NM-TRAN format by using arguments {{{torsten_src(time)}}}, {{{torsten_src(amt)}}}, {{{torsten_src(rate)}}}, {{{torsten_src(ii)}}}, {{{torsten_src(evid)}}}, {{{torsten_src(cmt)}}}, {{{torsten_src(addl)}}}, and
 {{{torsten_src(ss)}}} for events specification.
...
Function {{{torsten_src(pmx\_solve\_onecpt)}}} solves one-compartment PK
...
- ODE Parameters {{{torsten_src(theta)}}} consists of $CL$, $V_2$, $k_a$, in that order.
- The event arguments {{{torsten_src(time)}}}, {{{torsten_src(amt)}}}, {{{torsten_src(rate)}}}, {{{torsten_src(ii)}}}, {{{torsten_src(evid)}}}, {{{torsten_src(cmt)}}}, {{{torsten_src(addl)}}}, and
  {{{torsten_src(ss)}}}, describe the event schedule of the clinical
  trial. All arrays have the same length corresponding to the number of events.
- The model arguments, other than {{{torsten_src(theta)}}}, include
  + {{{torsten_src(biovar)}}}, the bioavailability fraction in each compartment
  + {{{torsten_src(tlag)}}}, the lag time in each compartment. 
- {{{torsten_src(theta)}}}, {{{torsten_src(biovar)}}}, {{{torsten_src(tlag)}}} may be either 
...
- The event arguments {{{torsten_src(time)}}}, {{{torsten_src(amt)}}}, {{{torsten_src(rate)}}}, {{{torsten_src(ii)}}}, {{{torsten_src(evid)}}}, {{{torsten_src(cmt)}}}, {{{torsten_src(addl)}}}, and
  {{{torsten_src(ss)}}}, describe the event schedule of the clinical
  trial. All arrays have the same length corresponding to the number of events.
- See section [[sec:onecpt]] regarding model arguments {{{torsten_src(theta)}}}, {{{torsten_src(biovar)}}}, and {{{torsten_src(tlag)}}}.
...
- {{{torsten_src(K)}}} contains system parameters. In the case of constant rate,
  =K= is the same for all events or an array of constant rate
  matrices. The length of the array is
  the number of events and the {{{torsten_src(i)}}} th element corresponds to the matrix at
  the interval ~[time[i-1], time[i]]~.
  Note that {{{torsten_src(K)}}} contains all the ODE parameters, so we no
  longer need {{{torsten_src(theta)}}}.
- See section [[sec:onecpt]] regarding model arguments {{{torsten_src(theta)}}},
  {{{torsten_src(biovar)}}}, and {{{torsten_src(tlag)}}}.
- The function returns a two-dimensional array of size =nt=
...
- User specifies $f(t, y(t))$ by defining =ODE_system=
  inside the {{{torsten_src(functions)}}} block (see section 19.2 of the
...
- {{{torsten_src(nCmt)}}} is the number of compartments (or, equivalently, the
...
- In the case of a multiple truncated infusion rate dosing regimen
  The bioavailability {{{torsten_src(biovar)}}} and the amount {{{torsten_src(amt)}}} cannot be passed as parameters.
- See section [[sec:onecpt]] regarding model arguments {{{torsten_src(theta)}}},
  {{{torsten_src(biovar)}}}, and {{{torsten_src(tlag)}}}.
