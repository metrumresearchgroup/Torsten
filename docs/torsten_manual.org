...
*** Note
    - Similar to single-subject solvers, three numerical integrator are provided:
      + {{{torsten_src(pmx\_solve_group\_adams)}}}: Adams-Moulton method,
      + {{{torsten_src(pmx\_solve_group\_bdf)}}}: Backward-differentiation formular,
      + {{{torsten_src(pmx\_solve_group\_rk45)}}}: Runge-Kutta 4/5 method.
    - With optional arguments indicated by square bracket, the following calls are allowed:
#+BEGIN_SRC stan
