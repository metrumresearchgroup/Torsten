...
\label{sec:orgb1e8a28}
\section*{Individuals}
\label{sec:orgb20c5f5}
...
or email us (\href{mailto:billg@metrumrg.com}{billg@metrumrg.com}, \href{mailto:yz@yizh.org}{yiz@metrumrg.com}) and we will help you help us!
...
started with Stan (\href{http://mc-stan.org/documentation}{http://mc-stan.org/documentation}).
In this section we go through the different functions Torsten adds to
Stan. The code for the examples can be found at the \texttt{example-models} folder. 

Torsten's functions are prefixed with {\small \color{MRGGreen} \texttt{\textbf{pmx\_}}}. 
...
\label{sec:org20edecc}
\index{One Compartment Model}
\label{sec:onecpt}
\subsection{Description}
\label{sec:orgd6c9e97}
...
\label{sec:orgef708eb}
\label{sec:twocpt}
\subsection{Description}
\label{sec:org83ed2b9}
...
\subsection{Arguments}
\label{sec:org4c8f2e7}
See Table \ref{tab:event_args} and Table \ref{tab:event_params}.

\subsection{Return value}
\label{sec:org07f3b4b}
...
\label{sec:org0f11adf}
\subsection{Description}
\label{sec:org1daac2e}
...
\subsection{Arguments}
\label{sec:org9849ef2}
\begin{itemize}
\item {\small \color{MRGGreen} \texttt{\textbf{K}}}
\label{sec:org562ae29}
...
\label{sec:org3b94f21}
\end{itemize}

\subsection{Return value}
\label{sec:org9c1e4eb}
...
\label{sec:org7fe2bce}
\subsection{Description}
\label{sec:orgd43a5fb}
...
\label{sec:org2bbfe64}
\index{coupled ODE Model}
\subsection{Description}
\label{sec:orga356fbd}
...
\end{minted}
\item {\small \color{MRGGreen} \texttt{\textbf{nCmt}}}
\label{sec:org66afc23}
The number of compartments. Equivalently, the dimension of the ODE system.
\item {\small \color{MRGGreen} \texttt{\textbf{rel\_tol}}}
\label{sec:orgb9ab6a2}
The relative tolerance for numerical integration, default to 1.0E-6.
\item {\small \color{MRGGreen} \texttt{\textbf{abs\_tol}}}
\label{sec:orgc0668dd}
The absolute tolerance for numerical integration, default to 1.0E-6.
\item {\small \color{MRGGreen} \texttt{\textbf{max\_step}}}
\label{sec:orgae8481f}
The maximum number of steps in numerical integration, default to \(10^6\).
\item See Table \ref{tab:event_args} and Table \ref{tab:event_params} for the rest of arguments.
\label{sec:org87d5fda}
\end{itemize}
\subsection{Return value}
\label{sec:org0223fdd}
...
\label{sec:orgcbce845}
\subsection{Description}
\label{sec:org16591c3}
...
when the corresponding event indicates infusion dosage.
\item {\small \color{MRGGreen} \texttt{\textbf{nCmt}}}
\label{sec:orgacc6223}
The number of compartments. Equivalently, the dimension of the ODE system.
\item {\small \color{MRGGreen} \texttt{\textbf{x\_r}}}
\label{sec:org47226d4}
2d arary real data to be passed to ODE RHS. If specified, its 1st
dimension should have the same size as {\small \color{MRGGreen} \texttt{\textbf{time}}}.
\item {\small \color{MRGGreen} \texttt{\textbf{x\_i}}}
\label{sec:orgdbda3cb}
2d arary integer data to be passed to ODE RHS. If specified, its 1st
dimension should have the same size as {\small \color{MRGGreen} \texttt{\textbf{time}}}.
\item {\small \color{MRGGreen} \texttt{\textbf{rel\_tol}}}
\label{sec:org1e4ed1e}
The relative tolerance for numerical integration, default to 1.0E-6.
\item {\small \color{MRGGreen} \texttt{\textbf{abs\_tol}}}
\label{sec:orgd83f0f9}
The absolute tolerance for numerical integration, default to 1.0E-6.
\item {\small \color{MRGGreen} \texttt{\textbf{max\_step}}}
\label{sec:orgf12395b}
The maximum number of steps in numerical integration, default to \(10^6\).
\item {\small \color{MRGGreen} \texttt{\textbf{as\_rel\_tol}}}
\label{sec:orgb79096f}
The relative tolerance for algebra solver for steady state solution, default to 1.0E-6.
\item {\small \color{MRGGreen} \texttt{\textbf{as\_abs\_tol}}}
\label{sec:orge3f4999}
The absolute tolerance for algebra solver for steady state solution, default to 1.0E-6.
\item {\small \color{MRGGreen} \texttt{\textbf{as\_max\_step}}}
\label{sec:orgd98f6b4}
The maximum number of interations in algebra solver for steady state solution, default to \(10^2\).
\end{itemize}
\subsection{Return value}
\label{sec:orgec1173f}
An \texttt{nCmt}-by-\texttt{nt} matrix, where \texttt{nt} is the total size of
events \(\sum_{i}n_i\).
\subsection{Note}
\label{sec:org53c42e2}
...
\label{sec:org682bfee}
\subsection{Description}
\label{sec:orgdb07812}
...
\end{minted}
\subsection{Arguments}
\label{sec:org555d5e1}
\label{sec:ode_func_args}
\begin{itemize}
\item {\small \color{MRGGreen} \texttt{\textbf{ODE\_rhs}}}
\label{sec:org2df6257}
...
\label{sec:org1daf446}
\index{PMX ODE group integrators}
\subsection{Description}
\label{sec:orge253761}
...
\chapter{Examples}
\label{sec:orgcefb040}
All the PMX models in this chapter can be found in
\texttt{Torsten/example-models} directory:
\begin{itemize}
\item \texttt{Torsten/example-models/pk2cpt} (Section \ref{sec:pk2cpt}).
\item \texttt{Torsten/example-models/pk2cpt\_linode} (Section \ref{sec:pk2cpt_linode}).
\item \texttt{Torsten/example-models/pk2cpt\_ode} (Section \ref{sec:pk2cpt_ode}).
\item \texttt{Torsten/example-models/FK\_coupled} (Section \ref{sec:fk_model}).
\item \texttt{Torsten/example-models/twocpt\_population} (Section \ref{sec:twocpt_population}).
\item \texttt{Torsten/example-models/lotka\_volterra\_ode\_group\_model} (Section \ref{sec:lotka_volterra}).
\item \texttt{Torsten/example-models/effCpt} (Section \ref{sec:effcpt_model}).
\item \texttt{Torsten/example-models/FribergKarlsson} (Section \ref{sec:fkpop_model}).
\end{itemize}

\section{Two-compartment model for single patient}
\label{sec:org9a8ba5d}
\label{sec:pk2cpt}
...
\mintinline[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}{model} block as they are identical to Sectiontion \ref{sec:pk2cpt}.
...
\label{sec:orgd85a736}
\label{sec:pk2cpt_ode}
Using {\small \color{MRGGreen} \texttt{\textbf{pmx\_solve\_rk45}}}, the following example fits a two-compartment model
...
therefore specify our model using a coupled solver function. We do not
...
\caption{\label{FK_model}Friberg-Karlsson semi-mechanistic Model.}
\end{figure}
...
\label{sec:org756a6ff}
\label{sec:twocpt_population}
Using {\small \color{MRGGreen} \texttt{\textbf{pmx\_solve\_group\_bdf}}}, the following example fits a
...
\label{sec:org83de000}
\label{sec:lotka_volterra}
Using {\small \color{MRGGreen} \texttt{\textbf{pmx\_integrate\_ode\_group\_rk45}}}, the following example fits
...
constant rate matrix and use {\small \color{MRGGreen} \texttt{\textbf{pmx\_solve\_linode}}}. Correct use of
Torsten requires the user pass the entire event history
(observation and dosing events) for an individual to the
function. Thus the Stan model shows the call to {\small \color{MRGGreen} \texttt{\textbf{pmx\_solve\_linode}}}
...
\ref{effcpt_ppc_resp_5mg}-\ref{effcpt_ppc_resp_study_2_20mg}) look
...
\label{sec:orgb2a1317}
\label{sec:fkpop_model}
We now return to the example in Section \ref{sec:fk_model} and extend
...
We use the {\small \color{MRGGreen} \texttt{\textbf{pmx\_solve\_group\_rk45}}} function to
...
\label{sec:org1d1ab64}
Torsten uses compiling flags to control some solver behavior. These flags can
be used in \texttt{cmdstan/make/local} file, or in \texttt{cmdstanr::set\_make\_local} function.
\begin{itemize}
...
\item {\small \color{MRGGreen} \texttt{\textbf{TORSTEN\_CVS\_JAC\_DQ=1}}} makes BDF and Adams
integrator use CVODES' default method to calculate
