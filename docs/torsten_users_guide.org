...
#+SUBTITLE: User's Guide @@latex:\\@@ @@html:<br>@@ (Torsten Version 0.88, Stan version 2.25.0)
#+LATEX_CLASS: amsbook
...
or email us ([[mailto:billg@metrumrg.com][billg@metrumrg.com]], [[mailto:yz@yizh.org][yiz@metrumrg.com]]) and we will help you help us!
...
Torsten's functions are prefixed with {{{torsten_src(pmx\_)}}}. 
...
*** Return value
    An =nCmt=-by-=nt= matrix, where =nt= is the size of {{{torsten_src(time)}}}.
*** Note
    - See section [[ref:sec:ode_func_note]] for different types of integrator and general guidance.
    - See section [[ref:sec:ode_func_note]] for comments on accuracy and tolerance.
    - The default values of {{{torsten_src(atol)}}},
      {{{torsten_src(rtol)}}}, and {{{torsten_src(max\_step)}}} are
...
& two-compartment for PK model, and {{{torsten_src(rk45)}}} &
{{{torsten_src(bdf)}}} integrator for nonlinear PD model.
*** Usage
#+BEGIN_SRC stan
...
*** Return value
    ...
    {{{torsten_src(time)}}}, and =nPk= equals to 2 in
    {{{torsten_src(pmx\_solve\_onecpt\_)}}} functions
    and 3 in {{{torsten_src(pmx\_solve\_twocpt\_)}}} functions.
...
arguments {{{torsten_src(time)}}}, {{{torsten_src(amt)}}}, {{{torsten_src(rate)}}}, {{{torsten_src(ii)}}},
{{{torsten_src(evid)}}}, {{{torsten_src(cmt)}}},
{{{torsten_src(addl)}}}, {{{torsten_src(ss)}}} specifies the entire
...
*** Arguments
**** {{{torsten_src(time)}}}, {{{torsten_src(amt)}}}, {{{torsten_src(rate)}}}, {{{torsten_src(ii)}}}, {{{torsten_src(evid)}}}, {{{torsten_src(cmt)}}}, {{{torsten_src(addl)}}}, {{{torsten_src(ss)}}}
     ...
     dimension. Take {{{torsten_src(evid)}}} for example. Let $N$ be the
     population size, then {{{torsten_src(evid[1,])}}} to
     {{{torsten_src(evid[n1,])}}} specifies events ID for subject 1,
     {{{torsten_src(evid[n1 + 1,])}}} to
     {{{torsten_src(evid[n1 + n2,])}}} for subject 2, etc. With $n_i$
     being the number of events for subject $i$, $i=1, 2, \dots, N$, the
     size of {{{torsten_src(evid)}}}'s first dimension is $\sum_{i}n_i$.
**** {{{torsten_src(len)}}} 
     The length of data for each subject within
     the above events arrays. The size of {{{torsten_src(len)}}} equals
...
\label{sec:ode_group_note}
    - Similar to single-subject solvers, three numerical integrator are provided:
      + {{{torsten_src(pmx\_solve\_group\_adams)}}}: Adams-Moulton method,
      + {{{torsten_src(pmx\_solve\_group\_bdf)}}}: Backward-differentiation formular,
      + {{{torsten_src(pmx\_solve\_group\_rk45)}}}: Runge-Kutta 4/5 method.
    - With optional arguments indicated by square bracket, the following calls are allowed:
#+BEGIN_SRC stan
...
**** {{{torsten_src(theta)}}}
     Parameters to be passed to {{{torsten_src(ODE\_rhs)}}} function.
**** {{{torsten_src(x\_r)}}}
     Real data to be passed to {{{torsten_src(ODE\_rhs)}}} function.
**** {{{torsten_src(x\_i)}}}
     Integer data to be passed to {{{torsten_src(ODE\_rhs)}}} function.
**** {{{torsten_src(rtol)}}}
     Relative tolerance, default to 1.e-6({{{torsten_src(rk45)}}}) and 1.e-8({{{torsten_src(adams)}}} and {{{torsten_src(bdf)}}}).
**** {{{torsten_src(atol)}}}
     Absolute tolerance, default to 1.e-6({{{torsten_src(rk45)}}}) and 1.e-8({{{torsten_src(adams)}}} and {{{torsten_src(bdf)}}}).
**** {{{torsten_src(max\_step)}}}
     Maximum number of steps allowed between neighboring time in {{{torsten_src(ts)}}},
     default to 100000.
*** Return value
    An =n=-by-=nd= 2d-array, where =n= is the size of {{{torsten_src(ts)}}}
    and =nd= the dimension of the system.
*** Note
\label{sec:ode_func_note}
    - Three numerical integrator are provided:
      + {{{torsten_src(pmx\_integrate\_ode\_adams)}}}: Adams-Moulton method,
      + {{{torsten_src(pmx\_integrate\_ode\_bdf)}}}: Backward-differentiation formular,
      + {{{torsten_src(pmx\_integrate\_ode\_rk45)}}}: Runge-Kutta 4/5 method.
    ...
    - All three integrators support adaptive stepping. To achieve
      ...
      to reduce step size and retry. The default values of {{{torsten_src(atol)}}},
      {{{torsten_src(rtol)}}}, and {{{torsten_src(max\_step)}}} are
      based on Stan's ODE functions and should not be considered as
      optimal. User should make problem-dependent
      decision on {{{torsten_src(rtol)}}} and {{{torsten_src(atol)}}},
      ...
      {{{torsten_src(atol)}}} greater than that threshold will avoid
...
**** {{{torsten_src(ts)}}}
     ...
     {{{torsten_src(ts)}}} of each subsystem concatenated.
**** {{{torsten_src(theta)}}}
     2d-array parameters to be passed to {{{torsten_src(ODE\_rhs)}}}
     function. Each row corresponds to one subsystem.
**** {{{torsten_src(x\_r)}}}
     2d-array real data to be passed to {{{torsten_src(ODE\_rhs)}}} function.
     Each row corresponds to one subsystem.
**** {{{torsten_src(x\_i)}}}
     2d-array integer data to be passed to {{{torsten_src(ODE\_rhs)}}} function.
     Each row corresponds to one subsystem.
**** {{{torsten_src(rtol)}}}
     Relative tolerance.
**** {{{torsten_src(atol)}}}
...
*** Return value
    An =n=-by-=nd= matrix, where =n= is the size of {{{torsten_src(ts)}}}
    and =nd= the dimension of the system.
*** Note
...
- =Torsten/example-models/pk2cpt= (Section [[ref:sec:pk2cpt]]).
- =Torsten/example-models/pk2cpt_linode= (Section [[ref:sec:pk2cpt_linode]]).
- =Torsten/example-models/pk2cpt_ode= (Section [[ref:sec:pk2cpt_ode]]).
- =Torsten/example-models/FK_coupled= (Section [[ref:sec:fk_model]]).
- =Torsten/example-models/twocpt_population= (Section [[ref:sec:twocpt_population]]).
- =Torsten/example-models/lotka_volterra_ode_group_model= (Section [[ref:sec:lotka_volterra]]).
- =Torsten/example-models/effCpt= (Section [[ref:sec:effcpt_model]]).
- =Torsten/example-models/FribergKarlsson= (Section [[ref:sec:fkpop_model]]).
...
Code below shows how Torsten function {{{torsten_src(pmx\_solve\_twocpt)}}} can be used to fit the above model.
...
Using {{{torsten_src(pmx\_solve\_linode)}}}, the following example fits a two-compartment model
with first order absorption. We omit src_stan[:exports code]{data} and
src_stan[:exports code]{model} block as they are identical to Sectiontion [[ref:sec:pk2cpt]].
...
Using {{{torsten_src(pmx\_solve\_rk45)}}}, the following example fits a two-compartment model
...
src_stan[:exports code]{model} block as they are identical to Section [[ref:sec:pk2cpt]].
...
Using {{{torsten_src(pmx\_solve\_group\_bdf)}}}, the following example fits a
two-compartment population model.
...
Using {{{torsten_src(pmx\_integrate\_ode\_group\_rk45)}}}, the following example fits
...
constant rate matrix and use {{{torsten_src(pmx\_solve\_linode)}}}. Correct use of
Torsten requires the user pass the entire event history
(observation and dosing events) for an individual to the
function. Thus the Stan model shows the call to {{{torsten_src(pmx\_solve\_linode)}}}
...
We use the {{{torsten_src(pmx\_solve\_group\_rk45)}}} function to
...
be used in =cmdstan/make/local= file, or in =cmdstanr::set_make_local= function.
- {{{torsten_src(TORSTEN\_MPI=1)}}} turns on within-chain parallelsation of MPI-enable
  functions. To use this option one must also point
  {{{torsten_src(TBB\_CXX\_TYPE)}}} to proper C compiler. See also section [[ref:sec:ode_group_note]] and [[ref:sec:ode_integ_group_note]].
- {{{torsten_src(TORSTEN\_CVS\_JAC\_DQ=1)}}} makes BDF and Adams
