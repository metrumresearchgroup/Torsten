...
\label{sec:org9ef3feb}
\section*{Individuals}
\label{sec:orgb28cae2}
...
\section{Changelog}
\label{sec:org8eef843}
\subsection{0.86 \textit{<2019-05-15 Wed>}}
\label{sec:orgeaf33b8}
\begin{itemize}
\item Added
\label{sec:org599aa37}
\begin{itemize}
...
\label{sec:orgd97e58f}
\begin{itemize}
\item Added
\label{sec:org8484fb8}
\begin{itemize}
\item Dosing rate as parameter
\end{itemize}
\item Changed
\label{sec:org8b4a3ea}
\begin{itemize}
...
\label{sec:org061d25d}
\begin{itemize}
\item Added
\label{sec:orgc9d21c7}
\begin{itemize}
...
\label{sec:orgac7829e}
\begin{itemize}
\item Added
\label{sec:orgfa02b96}
\begin{itemize}
...
\label{sec:org839ac05}
\begin{itemize}
\item Added
\label{sec:org229ce0e}
\begin{itemize}
...
models(Figure \ref{fig:orgba3dcfb}). The model obtains plasma concentrations of parent drug \(c=y_2/V_2\)
...
Torsten function \texttt{pmx\_solve\_twocpt} (see also Figure \ref{fig:orgba3dcfb}) solves two-compartment PK
...
\texttt{ss}, describe the event schedule of the clinical
trial. All arrays have the same length corresponding to the number of events.
\item See section \ref{org26e6065} regarding model arguments \texttt{theta}, \texttt{biovar}, and \texttt{tlag}.
...
two-compartment ODEs in \ref{orgd88cf75}, we simulate \(c\) according to:
\begin{align*}
...
The MCMC history plots(Figure \ref{fig:org9cb1b17})
...
data (Figure \ref{fig:org58ca8ad}) are in close agreement with the
...
summarized in Table \ref{tab:org840e99e} and Figure \ref{fig:orgad1b732}
are consistent with the values used for simulation.

\begin{table}[htbp]
\caption{\label{tab:org840e99e}
...
\item See section \ref{org26e6065} regarding model arguments \texttt{theta},
...
effect. In the current example, we use the two compartment model in section \ref{orgd88cf75} for
PK model.
...
neutrophils at a baseline value (Figure \ref{fig:orgd7a805e}). While in the
...
\label{sec:org7bc37f1}
\section{Effect Compartment Population Model}
\label{sec:orgc508836}
Here we expand the example in \ref{orgd88cf75} to a population model fitted to the
...
(Table \ref{tab:org7c00224}) and Figure \ref{effCptModelDens}).

\begin{table}[htbp]
\caption{\label{tab:org7c00224}
...
\label{sec:org2dfebd9}
We now return to the example in Section \ref{org346b8a4} and extend
...
estimation reflects the real value of the parameters (Table \ref{tab:orga343075} and Figure \ref{FKDens}).

\begin{table}[htbp]
\caption{\label{tab:orga343075}
