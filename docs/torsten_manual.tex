...
models(Figure \ref{fig:orgb40aec5}). The model obtains plasma concentrations of parent drug \(c=y_2/V_2\)
...
Torsten function \texttt{pmx\_solve\_twocpt} (see also Figure \ref{fig:orgb40aec5}) solves two-compartment PK
...
\texttt{ss}, describe the event schedule of the clinical
trial. All arrays have the same length corresponding to the number of events.
\item See section \ref{org1212574} regarding model arguments \texttt{theta}, \texttt{biovar}, and \texttt{tlag}.
...
two-compartment ODEs in \ref{org969eb5c}, we simulate \(c\) according to:
\begin{align*}
