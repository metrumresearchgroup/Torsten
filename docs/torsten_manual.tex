...
\label{sec:org61efa73}
\section*{Individuals}
\label{sec:orge8f93c7}
...
\section{Changelog}
\label{sec:org758fa40}
\subsection{0.87 \textit{<2019-07-26 Fri>}}
\label{sec:org6c834f6}
\begin{itemize}
\item Added
\label{sec:org00f4792}
\begin{itemize}
...
\label{sec:org880069f}
\begin{itemize}
\item Added
\label{sec:org38fd918}
\begin{itemize}
...
\label{sec:orgeaa1085}
\begin{itemize}
\item Added
\label{sec:org675fb1a}
\begin{itemize}
\item Dosing rate as parameter
\end{itemize}
\item Changed
\label{sec:org5b8baf2}
\begin{itemize}
...
\label{sec:org487c50f}
\begin{itemize}
\item Added
\label{sec:org7e83e9c}
\begin{itemize}
...
\label{sec:org5354a69}
\begin{itemize}
\item Added
\label{sec:org9e294fd}
\begin{itemize}
...
\label{sec:org0fbb652}
\begin{itemize}
\item Added
\label{sec:orga5d1f27}
\begin{itemize}
...
models(Figure \ref{fig:org2bec3d3}). The model obtains plasma concentrations of parent drug \(c=y_2/V_2\)
...
Torsten function \texttt{pmx\_solve\_twocpt} (see also Figure \ref{fig:org2bec3d3}) solves two-compartment PK
...
\texttt{ss}, describe the event schedule of the clinical
trial. All arrays have the same length corresponding to the number of events.
\item See section \ref{org2a45814} regarding model arguments \texttt{theta}, \texttt{biovar}, and \texttt{tlag}.
...
\item See section \ref{org2a45814} regarding model arguments \texttt{theta},
...
two-compartment ODEs in \ref{org83c0e51}, we simulate \(c\) according to:
\begin{align*}
...
The MCMC history plots(Figure \ref{fig:org4b4a355})
...
data (Figure \ref{fig:orgbd8fe8c}) are in close agreement with the
...
summarized in Table \ref{tab:org0572fff} and Figure \ref{fig:org64d3bcc}
...
effect. In the current example, we use the two compartment model in section \ref{org83c0e51} for
PK model.
...
neutrophils at a baseline value (Figure \ref{fig:org88672e8}). While in the
...
\label{sec:orgada7917}
Here we expand the example in \ref{org83c0e51} to a population model fitted to the
...
(Table \ref{tab:org02b267d}) and Figure \ref{effCptModelDens}).
...
\label{sec:org0615428}
We now return to the example in Section \ref{org8e79639} and extend
...
estimation reflects the real value of the parameters (Table \ref{tab:org4aaecce} and Figure \ref{FKDens}).
