...
\label{sec:org12579c0}
\section*{Individuals}
\label{sec:org29b6f6d}
...
\section{Changelog}
\label{sec:org615c402}
\subsection{0.87 \textit{<2019-07-26 Fri>}}
\label{sec:org5c6001f}
\begin{itemize}
\item Added
\label{sec:orgf7972eb}
\begin{itemize}
...
\label{sec:org3db5b11}
\begin{itemize}
\item Added
\label{sec:org2ad3ba1}
\begin{itemize}
...
\label{sec:org4f14fed}
\begin{itemize}
\item Added
\label{sec:org75979d3}
\begin{itemize}
\item Dosing rate as parameter
\end{itemize}
\item Changed
\label{sec:orgbee8617}
\begin{itemize}
...
\label{sec:orgc817de2}
\begin{itemize}
\item Added
\label{sec:orgab0d6f4}
\begin{itemize}
...
\label{sec:orgfc33bad}
\begin{itemize}
\item Added
\label{sec:orgcd6de5f}
\begin{itemize}
...
\label{sec:org100dd59}
\begin{itemize}
\item Added
\label{sec:org648fb9c}
\begin{itemize}
...
models(Figure \ref{fig:org4ab8aae}). The model obtains plasma concentrations of parent drug \(c=y_2/V_2\)
...
Torsten function \texttt{pmx\_solve\_twocpt} (see also Figure \ref{fig:org4ab8aae}) solves two-compartment PK
...
\texttt{ss}, describe the event schedule of the clinical
trial. All arrays have the same length corresponding to the number of events.
\item See section \ref{orgeb5775f} regarding model arguments \texttt{theta}, \texttt{biovar}, and \texttt{tlag}.
...
\item See section \ref{orgeb5775f} regarding model arguments \texttt{theta},
...
two-compartment ODEs in \ref{org0b3ebff}, we simulate \(c\) according to:
\begin{align*}
...
The MCMC history plots(Figure \ref{fig:orgbcf03f6})
...
data (Figure \ref{fig:org1656001}) are in close agreement with the
...
summarized in Table \ref{tab:orge6e3d33} and Figure \ref{fig:org378a873}
...
effect. In the current example, we use the two compartment model in section \ref{org0b3ebff} for
PK model.
...
neutrophils at a baseline value (Figure \ref{fig:orgd64b06d}). While in the
...
\label{sec:org6e0694c}
Here we expand the example in \ref{org0b3ebff} to a population model fitted to the
...
(Table \ref{tab:org590c623}) and Figure \ref{effCptModelDens}).
...
\label{sec:orgd9a6a97}
We now return to the example in Section \ref{org05d7d98} and extend
...
estimation reflects the real value of the parameters (Table \ref{tab:org450850f} and Figure \ref{FKDens}).
