...
real[nt, ncmt] = PKModelOneCpt(real[] time, real[] amt, real[] rate,
...
Torsten function \texttt{PKModelOneCpt} solves one-compartment PK
models(Figure \ref{fig:orgbe6371c}). The model obtains plasma concentrations of parent drug \(c=y_2/V_2\)
...
\begin{minted}[breaklines=true,fontsize=\footnotesize,breakanywhere=true]{stan}
real[nt, ncmt] = PKModelTwoCpt(real[] time, real[] amt, real[] rate,
                               real[] ii, int[] evid, int[] cmt,
                               real[] addl, int[] ss, real[] theta,
                               real[] biovar, real[] tlag)
\end{minted}

Torsten function \texttt{PKModelTwoCpt} (see also Figure \ref{fig:orgbe6371c}) solves two-compartment PK
