...
\newcommand\mrgsubtitle{\Large{User Manual} \linebreak (Torsten Version 0.86, Stan version 2.19.1)}
...
\large User Manual \\  (Torsten Version 0.86, Stan version 2.19.1)}
\hypersetup{
 pdfauthor={Yi Zhang},
...
\label{sec:org3bd4684}
\section*{Individuals}
\label{sec:org1f55dab}
...
\label{sec:orgf143c04}
\begin{itemize}
\item Current Torsten v0.86 is based on Stan v2.19.1.
...
\section{Changelog}
\label{sec:org4f1aa8e}
\subsection{0.86 \textit{<2019-05-15 Wed>}}
\label{sec:orgd0b6bc3}
\begin{itemize}
\item Added
\label{sec:orgdfdfa16}
\begin{itemize}
...
\label{sec:orgfc64b98}
\begin{itemize}
\item Added
\label{sec:org87178c9}
\begin{itemize}
\item Dosing rate as parameter
\end{itemize}
\item Changed
\label{sec:org9885463}
\begin{itemize}
...
\label{sec:org43f09c8}
\begin{itemize}
\item Added
\label{sec:orge0b74b1}
\begin{itemize}
...
\label{sec:org4768d23}
\begin{itemize}
\item Added
\label{sec:org5ed8bb1}
\begin{itemize}
...
\label{sec:org79cdc41}
\begin{itemize}
\item Added
\label{sec:org29d9583}
\begin{itemize}
...
Torsten. The latest version of Torsten (v0.86) is
...
models(Figure \ref{fig:org52cdf3b}). The model obtains plasma concentrations of parent drug \(c=y_2/V_2\)
...
Torsten function \texttt{pmx\_solve\_twocpt} (see also Figure \ref{fig:org52cdf3b}) solves two-compartment PK
...
\texttt{ss}, describe the event schedule of the clinical
trial. All arrays have the same length corresponding to the number of events.
\item See section \ref{orgca08e13} regarding model arguments \texttt{theta}, \texttt{biovar}, and \texttt{tlag}.
...
two-compartment ODEs in \ref{org82ea594}, we simulate \(c\) according to:
\begin{align*}
...
The MCMC history plots(Figure \ref{fig:org55dce00})
...
data (Figure \ref{fig:orga892edb}) are in close agreement with the
...
summarized in Table \ref{tab:org1ed6279} and Figure \ref{fig:org52c0be5}
are consistent with the values used for simulation.

\begin{table}[htbp]
\caption{\label{tab:org1ed6279}
...
\item See section \ref{orgca08e13} regarding model arguments \texttt{theta},
...
effect. In the current example, we use the two compartment model in section \ref{org82ea594} for
PK model.
...
neutrophils at a baseline value (Figure \ref{fig:org1783d98}). While in the
...
\label{sec:org195c383}
\section{Effect Compartment Population Model}
\label{sec:org91c65dd}
Here we expand the example in \ref{org82ea594} to a population model fitted to the
...
(Table \ref{tab:org394b251}) and Figure \ref{effCptModelDens}).

\begin{table}[htbp]
\caption{\label{tab:org394b251}
...
\label{sec:org668cd90}
We now return to the example in Section \ref{org22d7f52} and extend
...
estimation reflects the real value of the parameters (Table \ref{tab:org2bd6370} and Figure \ref{FKDens}).

\begin{table}[htbp]
\caption{\label{tab:org2bd6370}
